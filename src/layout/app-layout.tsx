...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['group-owner', 'payment-initiator', 'payment-contributor', 'group-member'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<GroupInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/groups` : null),
    () => getGroups({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Group',
      path: isTenantUser ? `/groups/view/${data?.[0]?.id}` : '/groups',
      entity: 'group',
